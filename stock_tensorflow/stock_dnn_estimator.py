...
class StockDnnEstimator(Stock):
    def dnnclassifier_estimator(self, testlen, ntrain, r, hidden_units, steps):
        ...
        for i in range(ntrain, ntest):
            ...
            traindatax = traindata.drop(['date', 'code', 'ratio'],
                                        1).values.astype('float32')
            traindatay = (traindata['ratio'] > r).astype(int).values
            testdatax = testdata.drop(['date', 'code', 'ratio'],
                                      1).values.astype('float32')

            # Specify that all features have real-value data
            feature_columns = [
                tf.contrib.layers.real_valued_column(
                    "", dimension=traindatax.shape[1])
            ]
    ...
    def dnnregressor_estimator(self, testlen, ntrain, hidden_units, steps):
        ...
        hsma = pd.DataFrame()
        store = pd.HDFStore('Test\\stock_tensorflow\\hsma_' + self.label +
                            '.h5')
        for i in range(ntrain, ntest):
            traindata = hsmadata[
                (hsmadata['date'] >= dates[(i - ntrain) * testlen])
    ...
    def widendeepclassifier_estimator(self, testlen, ntrain, r, hidden_units,
                                      steps):
    ...
    def widendeepregressor_estimator(self, testlen, ntrain, hidden_units,
                                     steps):
    ...
    def dnnclassifier(self, testlen, ntrain, r, hidden_units, steps):
        ...
        for i in range(ntrain, ntest):
            ...
            traindatay = (traindata['ratio'] >
                          r).values.astype('float32').reshape(
                              [traindata.shape[0], 1])
            traindatay = np.c_[traindatay, 1 - traindatay]
            testdatax = testdata.drop(['date', 'code', 'ratio'],
                                      1).values.astype('float32')
            testdatay = (testdata['ratio'] >
                         r).values.astype('float32').reshape(
                             [testdata.shape[0], 1])
