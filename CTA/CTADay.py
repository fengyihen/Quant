...
class CTADaySklearn(FutureDay.Future):
    ...
    def volpricebreak(self, code, length, m, fee):
        ...
        hsma['volp'] = (hsma.close - hsma.open) / (hsma.high - hsma.low + 1) * hsma.vol
        ...
        while True:            
            ...
            if ls == 'L':
                jarray = np.where(hsma.loc[i:(hsma.shape[0]-1), 'volp_sum'] < hsma.loc[i:(hsma.shape[0]-1), 'volp_mean'])[0]
                if len(jarray) == 0:
                    j = hsma.shape[0] - 1
                else:
                    j = i + jarray[0] + 1
