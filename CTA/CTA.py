...
class CTA(FutureMinute.FutureMinute):
        ...
        for d in hsma['date'].unique():
            ...
            if ls == 'L':
                if not any(hsmad.close.iloc[inpos:] < openprice * (1 - tpr)):
                    ...
                    idx = np.where(
                        hsmad.close.iloc[inpos:] < openprice * (1 - tpr))[0]
    ...
    def LS_turn(self, hsma, fee):
        ...
        while True:
            ...
            jarray = np.where((hsma['LS'].iloc[i:] != ls) &
                              (hsma['LS'].iloc[i:] != 'N'))[0]
            if len(jarray) == 0:
                j = hsma.shape[0] - 1
            else:
    ...
    def SMA_tpr_dayin(self, length, lr, tpr, wtime):
        ...
        for code in self.hsmaall['code'].unique():
            ...
            hsma['LS'] = 'N'
            temp = (hsma.MA1.shift(1) > hsma.MA2.shift(1)) & (
                hsma.close.shift(1) > hsma.open.shift(1))
    ...
    def SMA_ls(self, hsmaall, length, lr):
        ...
        for code in hsmaall['code'].unique():
            ...
            hsma['LS'] = 'N'
            temp = (hsma.MA1.shift(1) > hsma.MA2.shift(1)) & (
                hsma.MA1.shift(2) < hsma.MA2.shift(2)) & (hsma.MA2.shift(1) >
                                                          hsma.MA2.shift(2))
            hsma.loc[temp, 'LS'] = 'L'
            temp = (hsma.MA1.shift(1) < hsma.MA2.shift(1)) & (
                hsma.MA1.shift(2) > hsma.MA2.shift(2)) & (hsma.MA2.shift(1) <
    ...
    def RSI_tpr_dayin(self, hsmaall, length, rr, tpr, wtime):
        ...
        for code in hsmaall['code'].unique():
            ...
            hsma.loc[temp, 'LS'] = 'L'
            temp = (hsma.RSI.shift(1) < 100 - rr) & (hsma.RSI.shift(2) >
                                                     100 - rr)
            hsma.loc[temp, 'LS'] = 'S'
    ...
    def BBANDS_ls(self, hsmaall, length, nstd):
        ...
        for code in hsmaall['code'].unique():
            ...
            hsma['LS'] = 'N'
            temp = (hsma.close.shift(1) > hsma.upper.shift(1)) & (
                hsma.MA.shift(1) > hsma.MA.shift(2))
            hsma.loc[temp, 'LS'] = 'L'
            temp = (hsma.close.shift(1) < hsma.lower.shift(1)) & (
                hsma.MA.shift(1) < hsma.MA.shift(2))
            hsma.loc[temp, 'LS'] = 'S'
    ...
    def ADX_ls(self, hsmaall, length, rr):
        ...
        for code in hsmaall['code'].unique():
            ...
            hsma.loc[temp, 'LS'] = 'L'
            temp = (hsma.ADX.shift(1) < 100 - rr) & (hsma.ADX.shift(2) >
                                                     100 - rr)
            hsma.loc[temp, 'LS'] = 'S'
    ...
    def out_of_sample_month(self, strategy, ncode, ntrain, *tupleArg):
        ...
        for i in range(ntrain, len(months) - 2):
            ...
            testdata = self.hsmaall[
                (self.hsmaall['date'] >= months[i] * 100)
            ...
            portfoliot = portfoliot.sort_values(
                by='traderatio', ascending=False)
