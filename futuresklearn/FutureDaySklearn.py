...
class FutureSklearn(FutureDay.Future):
    def feature_selection(self, traindatax, traindatay, testdatax,
                          feature_sel):

        if feature_sel == "RFECV":
            estimator = LogisticRegression(solver='liblinear')
            selector = RFECV(estimator, step=1, cv=5)
            traindatax1 = pd.DataFrame(
                selector.fit_transform(traindatax, traindatay))
            traindatax1.columns = traindatax.columns[selector.get_support(
                True)]
        ...
        return traindatax1, testdatax1

    def linearregressor(self, testlen, ntrain, raw, timesteps, day,
                        feature_sel):
    ...
    def logisticmodel(self, testlen, ntrain, attr, timesteps, lengths, day,
                      feature_sel, direction, dayr):

        if attr == 'raw':
            hsmadata_x = self.hsmadata_raw_x(timesteps)
        elif attr == 'raw2':
            hsmadata_x = self.hsmadata_raw2_x(timesteps)
        elif attr == 'ta':
            hsmadata_x = self.hsmadata_ta_x(timesteps)
        else:
            pass
        hsmadata_y = self.hsmadata_y(day)
        hsmadataall = pd.merge(hsmadata_y, hsmadata_x)

        hsma = pd.DataFrame()
        for code in hsmadataall.code.unique():

            hsmadata = hsmadataall[hsmadataall.code == code]
            if hsmadata.shape[0] <= (ntrain + 1) * testlen:
                continue
            ...
            for i in range(ntrain, ntest):
                ...
                testdata = hsmadata[(hsmadata['date'] >= dates[i * testlen])
                                    & (hsmadata['date'] < dates[
                                        (i + 1) * testlen])].copy()
                print(testdata.date.max())
                if (traindata.shape[0] <
                        ntrain * testlen / 2) | (testdata.shape[0] == 0):
                    continue

                traindatax = traindata.drop(['date', 'code', 'ratio', 'oratio']
                            , 1)
                if direction == "L":
                    traindatay = traindata['ratio'] > day * dayr
                if direction == "S":
                    traindatay = traindata['ratio'] < -day * dayr
                testdatax = testdata[traindatax.columns]

                #在train中做变量筛选, sklearn.feature_selection中的方法
                traindatax1, testdatax1 = self.feature_selection(
                    traindatax, traindatay, testdatax, feature_sel)

                linearmodel = linear_model.LogisticRegression(solver='liblinear')
                linearmodel.fit(traindatax1, traindatay)
                testdata['pred'] = linearmodel.predict(testdatax1)
    ...
    def randomforestregressor(self, testlen, ntrain, attr, timesteps, lengths,
                              day, feature_sel, direction, dayr, ntrees,
                              nodes):
        ...
        for code in hsmadataall.code.unique():
            ...
            for i in range(ntrain, ntest):
                ...
                treemodel.fit(traindatax1, traindatay)
                testdata['predratio'] = treemodel.predict(testdatax1)

                hsma = pd.concat([hsma, testdata], ignore_index=True)

                hsma.to_hdf(
                    'Quant\\futuresklearn\\hsma_RF_' + self.label + '.h5',
                    'hsma')

        return (hsma)

    def extratreesregressor(self, testlen, timesteps, day, lr, ntrain, ntrees,
                            nodes):
    ...
    def extratreesregressor_doublerank(self, testlen, ntrain, ntrees, nodes,
                                       col1, tn1, ascending1, col2, tn2,
                                       ascending2):
        ...
        for i in range(ntrain, ntest):
            ...
            hsmatradeday = self.hsmatradedayregressor(condition, hsma, n, cr,
                                                      cta1)
            stocksklearn.tradestatlist(hsmatradeday)

        hsma.to_hdf(
            'Test\\stocksklearn\\hsma_extratreesregressor_doublerank_' +
            self.label + '.h5', 'hsma')
    ...
    def GBRTregressor(self, testlen, ntrain, ntrees, nodes):
        ...
        for i in range(ntrain, ntest):
            ...
            treemodel = GradientBoostingRegressor(
                n_estimators=ntrees,
                min_samples_split=nodes * 2,
                min_samples_leaf=nodes)
            treemodel.fit(traindatax, traindatay)
            ...
            hsma = pd.concat([hsma, testdata], ignore_index=True)
        hsma.to_hdf(
            'Test\\stocksklearn\\hsma_GBRTregressor_' + self.label + '.h5',
            'hsma')
    ...
    def MLPRegressor(self, testlen, ntrain, *hidden_layer_sizes):
        ...
        hsma = pd.DataFrame()
        store = pd.HDFStore('Test\\stocksklearn\\hsma_MLPRegressor_' +
                            self.label + '.h5')
        for i in range(ntrain, ntest):
            traindata = hsmadata[
                (hsmadata['date'] >= dates[(i - ntrain) * testlen])
    ...
    def logistic_binandwoe(self, testlen, ntrain, feature_sel, varthreshold,
                           cv, binn, bq, r0):
        ...
        for i in range(ntrain, ntest):
            ...
            testdatax = testdata[traindatax.columns]
            traindatax, testdatax = self.binandwoe_traintest(
                traindatax, traindatay, testdatax, binn, bq, r0)
            ...
            hsma = pd.concat([hsma, testdata], ignore_index=True)
            hsma.to_hdf(
                'Test\\stocksklearn\\hsma_logisticbinandwoe_' + self.label +
                '.h5', 'hsma')
    ...
    def extratreesregressor_cluster(self, testlen, ntrain, ntrees, nodes,
        ...
        for i in range(ntrain, ntest):
            ...
            trainlabel, testlabel = self.ClusterModel(
                traindatax, testdatax, columns, cmodel, ncluster)
            ...
            for cl in testlabel.unique():
                ...
                testdata.ix[testlabel == cl, 'predratio'] = treemodel.predict(
                    testdataxcl)
    ...
    def svcstkmodel(self, testlen, ntrain, kernel, dayr):
        ...
        for i in range(ntrain, ntest):
            for code in codelist:
                ...
                traindata = hsmadata1[
                    (hsmadata1['date'] >= dates[(i - ntrain) * testlen])
                    & (hsmadata1['date'] < dates[i * testlen - self.day]
                       )].copy()
                testdata = hsmadata1[(hsmadata1['date'] >= dates[i * testlen])
                                     & (hsmadata1['date'] < dates[
                                         (i + 1) * testlen])].copy()
        ...
        hsma.to_hdf(
            'Test\\stocksklearn\\hsma_svcstkmodel_' + self.label + '.h5',
            'hsma')
    ...
    def extratreesregressor_ind(self, testlen, ntrain, ntrees):
        ...
        for i in range(ntrain, ntest):
            for ind in stocksector['SWN'].unique():
                ...
                traindata = hsmadata[
                    (hsmadata['date'] >= dates[(i - ntrain) * testlen])
                    & (hsmadata['date'] < dates[i * testlen - self.day]) &
                    (hsmadata['SWN'] == ind)].copy()
                testdata = hsmadata[(hsmadata['date'] >= dates[i * testlen])
                                    & (hsmadata['date'] < dates[
                                        (i + 1) * testlen]) &
                                    (hsmadata['SWN'] == ind)].copy()
    ...
    def svc_index(self, testlen, ntrain, index, kernel, dayr):
        ...
        if self.mtype != 'None':
            hsmadata_index = hsmadata_index[
                hsmadata_index['date'] >= self.hsmadata['date'].min()]
        ...
        for i in range(ntrain, ntest):
            traindata = hsmadata_index[
                (hsmadata_index['date'] >= dates[(i - ntrain) * testlen])
                & (hsmadata_index['date'] < dates[i * testlen - self.day]
                   )].copy()
            testdata = hsmadata_index[
                (hsmadata_index['date'] >= dates[i * testlen])
                & (hsmadata_index['date'] < dates[(i + 1) * testlen])].copy()
            print(testdata.date.max())
    ...
    def indexhighlow1day(self, hsmaindex, pr, mr):
        ...
        hsma0 = pd.read_csv("raw data\\stock\\indexday_" + hsmaindex.code[0] +
                            ".csv")
        hsma0 = hsma0[hsma0['vol'] > 0].sort_values(by='date').copy()

        hsma0['high1ratio'] = hsma0['open'].shift(
            -(self.day + 1)) / hsma0['high'] - 1
        hsma0.ix[hsma0['open'].shift(-1) > hsma0['high'],
                 'high1ratio'] = hsma0['open'].shift(
                     -(self.day + 1)) / hsma0['open'].shift(-1) - 1
        hsma0.ix[hsma0['high'].shift(-1) < hsma0['high'], 'high1ratio'] = 0
        hsma0.ix[hsma0['open'].shift(-1) > hsma0['close'] *
                 (1 + mr), 'high1ratio'] = 0

        hsma0['low1ratio'] = 1 - hsma0['open'].shift(
            -(self.day + 1)) / hsma0['low']
        hsma0.ix[hsma0['open'].shift(-1) < hsma0['low'],
                 'low1ratio'] = 1 - hsma0['open'].shift(
                     -(self.day + 1)) / hsma0['open'].shift(-1)
        ...
        hsmaindex['ratio'] = 0
        hsmaindex.ix[hsmaindex.predratio > pr, 'ratio'] = hsmaindex.ix[
            hsmaindex.predratio > pr, 'high1ratio']
        hsmaindex.ix[hsmaindex.predratio < -pr, 'ratio'] = hsmaindex.ix[
            hsmaindex.predratio < -pr, 'low1ratio']
    ...
    def closer1day(self, hsma, pr, mr, cr):  #cr < mr
        ...
        for i in buypoints.index:
            buypoints[i] = math.ceil(
                buypoints[i] / self.minpoint) * self.minpoint
        hsma0['high1ratio'] = hsma0['open'].shift(
            -(self.day + 1)) / buypoints - 1
        hsma0.ix[hsma0['open'].shift(-1) >
                 (hsma0['close'] *
                  (1 + cr)), 'high1ratio'] = hsma0['open'].shift(
                      -(self.day + 1)) / hsma0['open'].shift(-1) - 1
        hsma0.ix[hsma0['high'].shift(-1) <
                 (hsma0['close'] * (1 + cr)), 'high1ratio'] = 0
        hsma0.ix[hsma0['open'].shift(-1) > hsma0['close'] *
                 (1 + mr), 'high1ratio'] = 0

        sellpoints = (hsma0['close'] * (1 - cr))
        for i in sellpoints.index:
            sellpoints[i] = math.floor(
                sellpoints[i] / self.minpoint) * self.minpoint
        hsma0['low1ratio'] = 1 - hsma0['open'].shift(
            -(self.day + 1)) / sellpoints
        hsma0.ix[hsma0['open'].shift(-1) <
                 (hsma0['close'] *
                  (1 - cr)), 'low1ratio'] = 1 - hsma0['open'].shift(
                      -(self.day + 1)) / hsma0['open'].shift(-1)
        hsma0.ix[hsma0['low'].shift(-1) >
                 (hsma0['close'] * (1 - cr)), 'low1ratio'] = 0
        hsma0.ix[hsma0['open'].shift(-1) < hsma0['close'] *
                 (1 - mr), 'low1ratio'] = 0
        ...
        hsma['ratio'] = 0
        hsma.ix[(hsma.predratio > pr) &
                (hsma0['high1ratio'] != 0), 'ratio'] = hsma.ix[
                    (hsma.predratio > pr) &
                    (hsma0['high1ratio'] != 0), 'high1ratio'] - self.fee
        hsma.ix[(hsma.predratio < -pr) &
                (hsma0['low1ratio'] != 0), 'ratio'] = hsma.ix[
                    (hsma.predratio < -pr) &
                    (hsma0['low1ratio'] != 0), 'low1ratio'] - self.fee
    ...
    def closer1day_ROC(self, hsma, d, pr, mr, cr):  #cr < mr
        ...
        hsma0['high1ratio'] = hsma0['open'].shift(-(self.day + 1)) / (
            hsma0['close'] * (1 + cr)) - 1
        hsma0.ix[hsma0['open'].shift(-1) >
                 (hsma0['close'] *
                  (1 + cr)), 'high1ratio'] = hsma0['open'].shift(
        ...
        hsma0['low1ratio'] = 1 - hsma0['open'].shift(-(self.day + 1)) / (
            hsma0['close'] * (1 - cr))
        hsma0.ix[hsma0['open'].shift(-1) <
                 (hsma0['close'] *
                  (1 - cr)), 'low1ratio'] = 1 - hsma0['open'].shift(
    ...
    def closer1dayin(self, hsmaindex, pr, mr, cr):  #cr < mr
        ...
        hsma0['high1ratio'] = hsma0['close'].shift(-self.day) / (
            hsma0['close'] * (1 + cr)) - 1
        hsma0.ix[hsma0['open'].shift(-1) >
                 (hsma0['close'] *
                  (1 + cr)), 'high1ratio'] = hsma0['open'].shift(
                      -self.day) / hsma0['open'].shift(-1) - 1
        hsma0.ix[hsma0['high'].shift(-1) <
                 (hsma0['close'] * (1 + cr)), 'high1ratio'] = 0
        hsma0.ix[hsma0['open'].shift(-1) > hsma0['close'] *
                 (1 + mr), 'high1ratio'] = 0

        hsma0['low1ratio'] = 1 - hsma0['open'].shift(-self.day) / (
            hsma0['close'] * (1 - cr))
        hsma0.ix[hsma0['open'].shift(-1) <
                 (hsma0['close'] *
                  (1 - cr)), 'low1ratio'] = 1 - hsma0['open'].shift(
                      -self.day) / hsma0['open'].shift(-1)
        hsma0.ix[hsma0['low'].shift(-1) >
                 (hsma0['close'] * (1 - cr)), 'low1ratio'] = 0
        hsma0.ix[hsma0['open'].shift(-1) < hsma0['close'] *
                 (1 - mr), 'low1ratio'] = 0
    ...
    def closer1day_base(self, mr, cr):  #cr < mr
        ...
        hsma0.ix[hsma0['high1ratio'] != 0, 'ratio'] = hsma0.ix[
            hsma0['high1ratio'] != 0, 'ratio'] - self.fee
        hsma0.ix[hsma0['low1ratio'] != 0, 'ratio'] = hsma0.ix[
            hsma0['low1ratio'] != 0, 'ratio'] - self.fee
    ...
    def closer1day_bollinger(self, malen, mr, cr):  #cr < mr
        ...
        hsma0.ix[hsma0['open'].shift(-1) > buypoints, 'high1ratio'] = hsma0[
            'open'].shift(-2) / hsma0['open'].shift(-1) - 1
        ...
        hsma0.ix[hsma0['open'].shift(-1) < sellpoints,
                 'low1ratio'] = 1 - hsma0['open'].shift(
                     -2) / hsma0['open'].shift(-1)
    ...
    def closer1dayin_base(self, mr, cr):  #cr < mr
        ...
        hsma0.ix[hsma0['open'].shift(-1) >
                 (hsma0['close'] *
                  (1 + cr)), 'high1ratio'] = hsma0['close'].shift(
                      -self.day) / hsma0['open'].shift(-1) - 1
        hsma0.ix[hsma0['high'].shift(-1) <
                 (hsma0['close'] * (1 + cr)), 'high1ratio'] = 0
        hsma0.ix[hsma0['open'].shift(-1) > hsma0['close'] *
        ...
        hsma0['low1ratio'] = 1 - hsma0['close'].shift(-self.day) / (
            hsma0['close'] * (1 - cr))
        hsma0.ix[hsma0['open'].shift(-1) <
                 (hsma0['close'] *
                  (1 - cr)), 'low1ratio'] = 1 - hsma0['close'].shift(
                      -self.day) / hsma0['open'].shift(-1)
        hsma0.ix[hsma0['low'].shift(-1) >
                 (hsma0['close'] * (1 - cr)), 'low1ratio'] = 0
        hsma0.ix[hsma0['open'].shift(-1) < hsma0['close'] *
    ...
    def MA(self, malen):
        ...
        for i in range(1, hsma.shape[0]):
            ...
            #开仓
            if hsma.ix[i - 1, 'LS'] == 'N':
                if (hsma.ix[i - 1, 'close'] > hsma.ix[i - 1, 'MA']) & (
                        hsma.ix[i, 'high'] >= hsma.ix[i - 1, 'MA']):
                    hsma.ix[i, 'buy'] = max(
                        hsma.ix[i, 'open'],
                        math.ceil(hsma.ix[i - 1, 'MA'] / self.minpoint) *
                        self.minpoint)
                    hsma.ix[i, 'fee'] = hsma.ix[i, 'buy'] * self.fee / 2
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i, 'close'] - hsma.ix[i, 'buy']
                    hsma.ix[i, 'LS'] = 'L'
                if (hsma.ix[i - 1, 'close'] < hsma.ix[i - 1, 'MA']) & (
                        hsma.ix[i, 'low'] <= hsma.ix[i - 1, 'MA']):
                    hsma.ix[i, 'sell'] = min(
                        hsma.ix[i, 'open'],
                        math.floor(hsma.ix[i - 1, 'MA'] / self.minpoint) *
                        self.minpoint)
                    hsma.ix[i, 'fee'] = hsma.ix[i, 'sell'] * self.fee / 2
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i, 'sell'] - hsma.ix[i, 'close']
                    hsma.ix[i, 'LS'] = 'S'
            ...
            if hsma.ix[i - 1, 'LS'] == 'L':
                if (hsma.ix[i, 'low'] <= hsma.ix[i - 1, 'MA']):
                    ...
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i, 'sell'] - hsma.ix[
                            i - 1, 'close'] + hsma.ix[i, 'sell'] - hsma.ix[
                                i, 'close']
                    hsma.ix[i, 'LS'] = 'S'
                else:
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] + hsma.ix[
                        i, 'close'] - hsma.ix[i - 1, 'close']
                    hsma.ix[i, 'LS'] = 'L'
            if hsma.ix[i - 1, 'LS'] == 'S':
                if (hsma.ix[i, 'high'] >= hsma.ix[i - 1, 'MA']):
                    ...
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i - 1, 'close'] - hsma.ix[
                            i, 'buy'] + hsma.ix[i, 'close'] - hsma.ix[i, 'buy']
                    hsma.ix[i, 'LS'] = 'L'
                else:
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] + hsma.ix[
                        i - 1, 'close'] - hsma.ix[i, 'close']
                    hsma.ix[i, 'LS'] = 'S'

        hsma['simpleratio'] = hsma['net'] / hsma.ix[0, 'net'] - 1
        hsma['dayratio'] = (
            hsma['net'] - hsma.net.shift(1)) / hsma.close.shift(1)
    ...
    def MAcross(self, malen1, malen2):
        ...
        for i in range(1, hsma.shape[0]):
            ...
            #开仓
            if hsma.ix[i - 1, 'LS'] == 'N':
                if (hsma.ix[i - 1, 'MA1'] > hsma.ix[i - 1, 'MA2']) & (
                        hsma.ix[i, 'high'] >= hsma.ix[i - 1, 'MA1']):
                    hsma.ix[i, 'buy'] = max(
                        hsma.ix[i, 'open'],
                        math.ceil(hsma.ix[i - 1, 'MA1'] / self.minpoint) *
                        self.minpoint)
                    hsma.ix[i, 'fee'] = hsma.ix[i, 'buy'] * self.fee / 2
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i, 'close'] - hsma.ix[i, 'buy']
                    hsma.ix[i, 'LS'] = 'L'
                if (hsma.ix[i - 1, 'MA1'] < hsma.ix[i - 1, 'MA2']) & (
                        hsma.ix[i, 'low'] <= hsma.ix[i - 1, 'MA1']):
                    hsma.ix[i, 'sell'] = min(
                        hsma.ix[i, 'open'],
                        math.floor(hsma.ix[i - 1, 'MA1'] / self.minpoint) *
                        self.minpoint)
                    hsma.ix[i, 'fee'] = hsma.ix[i, 'sell'] * self.fee / 2
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i, 'sell'] - hsma.ix[i, 'close']
            ...
            if hsma.ix[i - 1, 'LS'] == 'L':
                if (hsma.ix[i - 1, 'MA1'] < hsma.ix[i - 1, 'MA2']) & (
                    ...
                    hsma.ix[i, 'fee'] = hsma.ix[i, 'sell'] * self.fee / 2
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i, 'sell'] - hsma.ix[i -
                                                                 1, 'close']
                    hsma.ix[i, 'LS'] = 'N'
                else:
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] + hsma.ix[
            ...
            if hsma.ix[i - 1, 'LS'] == 'S':
                if (hsma.ix[i - 1, 'MA1'] > hsma.ix[i - 1, 'MA2']) & (
                    ...
                    hsma.ix[i, 'fee'] = hsma.ix[i, 'buy'] * self.fee
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                        i, 'fee'] + hsma.ix[i - 1, 'close'] - hsma.ix[i, 'buy']
                    hsma.ix[i, 'LS'] = 'N'
                else:
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] + hsma.ix[
                        i - 1, 'close'] - hsma.ix[i, 'close']
    ...
    def MA_ADX(self, malen, s):
        ...
        for i in range(1, hsma.shape[0]):
            ...
            if hsma.ix[i - 1, 'LS'] == 'N':
                if (hsma.ix[i - 1, 'close'] > hsma.ix[i - 1, 'MA']) & (
                        hsma.ix[i, 'high'] >=
                        hsma.ix[i - 1, 'MA']) & (hsma.ix[i - 1, 'ADX'] >= s):
                    hsma.ix[i, 'buy'] = max(
                        hsma.ix[i, 'open'],
                        math.ceil(hsma.ix[i - 1, 'MA'] / self.minpoint) *
                        self.minpoint)
            ...
            if hsma.ix[i - 1, 'LS'] == 'L':
                if hsma.ix[i, 'low'] <= hsma.ix[i - 1, 'MA']:
                    if (hsma.ix[i - 1, 'close'] < hsma.ix[i - 1, 'MA']) & (
                        ...
                        hsma.ix[i, 'fee'] = hsma.ix[i, 'sell'] * self.fee / 2
                        hsma.ix[i, 'net'] = hsma.ix[i, 'net'] - hsma.ix[
                            i, 'fee'] + hsma.ix[i, 'sell'] - hsma.ix[i - 1,
                                                                     'close']
                        hsma.ix[i, 'LS'] = 'N'
                else:
                    hsma.ix[i, 'net'] = hsma.ix[i, 'net'] + hsma.ix[
    ...
    def MA_optimizer(self, malens):
        ...
        tradestatlist = tradestatlist.sort_values(
            by='RRR', ascending=False).copy()
        print(tradestatlist)
