...
class Future():
    ...
    def hsma0(self, code, cycle):
        ...
        hsma0 = pd.read_csv("raw data\\stock\\futures\\min1\\" + code + "_" +
                            cycle + ".csv")
        hsma0.columns = [
            'datetime', 'open', 'high', 'low', 'close', 'vol', 'openint'
        ]
        hsma0['datetime'] = hsma0['datetime'].apply(
            lambda x: datetime.datetime.strptime(x, "%Y/%m/%d %H:%M"))
        hsma0['date'] = hsma0['datetime'].apply(
            lambda x: x.year * 10000 + x.month * 100 + x.day)
        hsma0['time'] = hsma0['datetime'].apply(
            lambda x: x.hour * 100 + x.minute)
        hsma0['code'] = code
        hsma0 = hsma0[[
            'date', 'time', 'code', 'open', 'high', 'low', 'close', 'vol',
            'openint'
        ]]
    ...
    def hsmaall(self, cycle):
        ...
        for code in self.feelist.keys():
            if os.path.exists("raw data\\stock\\futures\\min1\\" + code + "_" +
                              cycle + ".csv"):
                print(code)
    ...
    def hsmadata(self, llen, slen):  #code 只能是商品
        ...
        for code in self.hsmaall.code.unique():
            ...
            hsma0['smar'] = hsma0['close'] / talib.SMA(
                hsma0.close.values, timeperiod=llen) - 1
            hsma0['VAR'] = talib.STDDEV(
                hsma0.close.values, timeperiod=llen) / hsma0['close'] - 1
            hsma0['timepos'] = 0
    ...
    def hsmatrade(self, hsma, wtime, tpr):
        ...
        if not 'LS' in hsma.columns:
            hsma['LS'] = 'N'
            hsma.loc[(hsma.prob_long > 0.5) &
                     (hsma.prob_short < 0.5), 'LS'] = 'L'
            hsma.loc[(hsma.prob_long < 0.5) &
                     (hsma.prob_short > 0.5), 'LS'] = 'S'
        ...
        for code in hsma['code'].unique():
            ...
            for d in hsmacode['date'].unique():
                ...
                if ls == 'L':
                    if not any(
                            hsmad.close.iloc[inpos:] < openprice * (1 - tpr)):
                        outpos = hsmad.shape[0] - 1
                    else:
                        idx = np.where(hsmad.close.iloc[inpos:] < openprice *
                                       (1 - tpr))[0]
    ...
    def traderatiostat(self, hsmatrade):
        ...
        yearratio = pd.merge(
            yearmean, yearcount, left_index=True, right_index=True)
        yearratio = pd.merge(
            yearratio, yearsum, left_index=True, right_index=True)
        ...
        monthratio = pd.merge(
            monthmean, monthcount, left_index=True, right_index=True)
        monthratio = pd.merge(
            monthratio, monthsum, left_index=True, right_index=True)
        ...
        coderatio = pd.merge(
            codemean, codecount, left_index=True, right_index=True)
        coderatio = pd.merge(
            coderatio, codesum, left_index=True, right_index=True)
