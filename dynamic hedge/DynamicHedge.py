...
class DynamicHedge(object):
    ...
    def hsmadata(self):
        ...
        if self.datataq:
            if self.market == 'market':
                LTTAQday = pd.read_csv(
                    "strategy data\\LTTAQdayMarketMAshort.csv")
            ...
            LTdayall = LTdayall.rename(
                columns={"ooratio" + str(self.day): 'closeratio'})
        ...
        if not self.absratio:
            ...
            hsma['closeratio'] = hsma['closeratio'] - hsma["ooratio" +
                                                           str(self.day)]
            hsma = hsma.drop(["ooratio" + str(self.day)], 1)

        if self.market == 'market':
            LTdayTTR = pd.read_csv(
                "strategy data\\LTdayTTR0Market_length7_sd2.csv")
        else:
            LTdayTTR = pd.read_csv("strategy data\\LTdayTTR0_length7_sd2.csv")
        LTdayTTR = LTdayTTR[[
            "code", "date", "adx", "mfi", "rsi", "volatility", "roc", "cci"
        ]]
        LTdayTTR.columns = [
            "code", "date", "adx7", "mfi7", "rsi7", "volatility7", "roc7",
            "cci7"
        ]
        ...
        if self.market == 'market':
            LTdayTTR = pd.read_csv(
                "strategy data\\LTdayTTR0Market_length14_sd2.csv")
        else:
        ...
        LTdayTTR.columns = [
            "code", "date", "adx14", "mfi14", "rsi14", "volatility14", "roc14",
            "cci14"
        ]
        hsma = pd.merge(hsma, LTdayTTR)
        ...
        if self.market == 'SH':
            ...
            LTTRANSACTIONday = pd.read_csv(
                "strategy data\\LTTRANSACTIONday.csv")
    ...
    def extratreesregressor_conditionrank(self, testlen, ntrain, ntrees, nodes,
                                          col, tn):
        hsmadata = self.hsmadata
        dates = pd.Series(hsmadata['date'].unique()).sort_values()
        dates.index = range(0, len(dates))
    ...
    def hsmatradeday(self, hsma, n, idxma, stockFF):
        ...
        hsmatradeday = hsmatrade.groupby(
            ['date'], as_index=False)[['closeratio']].mean()
        ...
        if idxma != 0:
            ...
            for i in hsmatradeday.index:
                if hsmatradeday.ix[i, 'idxclose'] < hsmatradeday.ix[i, 'idxma']:
                    ...
                    hsmatradeday.ix[i:min(
                        max(hsmatradeday.index), i + self.day -
                        1), 'position'] += 1 / self.day

            hsmatradeday['FFcloseratio'] = hsmatradeday[
                'FFcloseratio'] * hsmatradeday['position']

        hsmatradeday['stockratio'] = hsmatradeday['stockcloseratio'].cumsum()
        hsmatradeday['FFratio'] = hsmatradeday['FFcloseratio'].cumsum()

        hsmatradeday['ratio'] = (
            hsmatradeday['stockcloseratio'] + hsmatradeday['FFcloseratio']) / 2
        hsmatradeday['cumratio'] = (
            hsmatradeday['stockratio'] + hsmatradeday['FFratio']) / 2
    ...
    def hsmatradeclassifier(self, hsma, minn):
        ...
        for d in dates:
            ...
            if hsmad.shape[0] < minn:
                ...
                hsmad['closeratio'] = hsmad['closeratio'] * (
                    1 / hsmad.shape[0])
    ...
    def hsmatradedayclassifier(self, hsma, minn, idxma):
        ...
        hsmatradeday = hsmatrade.groupby(['date'],
                                         as_index=False)[['closeratio']].sum()
        hsmatradeday['stockcloseratio'] = hsmatradeday['closeratio'] / self.day
    ...
    def hsmapredmono(self, hsma, qn=10):
        ...
        for d in dates:
            ...
            for i in range(0, qn):
                tempi = temp[
                    (temp['predratio'] >= temp['predratio'].quantile(i / qn))
                    & (temp['predratio'] <= temp['predratio'].quantile(
                        (i + 1) / qn))].copy()
                tempi['quantile'] = i + 1
                hsmaall = pd.concat(
                    [hsmaall, tempi[['date', 'closeratio', 'quantile']]],
                    ignore_index=True)

        hsmapredmono = hsmaall.groupby(['quantile'],
                                       as_index=False)[['closeratio']].mean()
        return (hsmapredmono)
    ...
    def tradestat(self, hsmatradeday, stockFF):
        ...
        tradestat = pd.DataFrame({
            'startdate': [min(hsmatradeday['date'])],
            'enddate': [max(hsmatradeday['date'])]
        })
        tradestat['ratio'] = hsmatradeday.ix[hsmatradeday.shape[0] -
                                             1, 'cumratio']
        ...
        hsmatradeday['year'] = 0
        for i in hsmatradeday.index:
            hsmatradeday.ix[i, 'year'] = int(
                str(hsmatradeday.ix[i, 'date'])[0:4])
            mdd1 = hsmatradeday.ix[i, 'cumratio'] - min(
                hsmatradeday.ix[i:, 'cumratio'])
            if mdd1 > mdd:
                mdd = mdd1
        ...
        stockFF = pd.read_csv("strategy data\\" + stockFF + ".csv")
        totdays = sum((stockFF['date'] >= min(hsmatradeday['date'])) &
                      (stockFF['date'] <= max(hsmatradeday['date'])))
        tradestat['yearratio'] = tradestat['ratio'] / totdays * 252
        tradestat['mdd'] = mdd
        tradestat['mdddate'] = mdddate
        tradestat['RRR'] = tradestat['yearratio'] / tradestat['mdd']

        tradestat['sharpratio'] = hsmatradeday['ratio'].mean(
        ) / hsmatradeday['ratio'].std() * 252**0.5
    ...
    def tradestatlist(self, hsma, n, idxma=0):  #n=200,100,50
        ...
        tradestatlist.to_csv(
            "Test\\dynamic hedge\\testresult\\DynamicHedge_" + self.label +
            ".csv",
            index=False)
