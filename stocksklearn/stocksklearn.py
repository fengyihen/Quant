...
class StockSklearn(Stock.Stock):
    ...
    def extratreestrade_op(self, testlen, ntrain, ntrees, nodes, n, testdate,
                           label):
        hsmadata = self.hsmadata
        dates = pd.Series(hsmadata['date'].unique()).sort_values()
        dates.index = range(0, len(dates))
        ...
        if self.datataq:
            testdata = pd.read_csv(
                "Test\\dynamic hedge\\testresult\\DynamicHedge_testdata_taq.csv"
            )  #market='market'
        else:
            testdata = pd.read_csv(
                "Test\\dynamic hedge\\testresult\\DynamicHedge_testdata_notaq.csv"
            )  #market='market'
        ...
        hsmad.to_csv(
            "Test\\dynamic hedge\\testresult\\DynamicHedge_op_" + str(testdate)
            + "_" + label + ".csv",
            index=False)
