...
class FutureLogistic(FutureMinute.Future):
    ...
    def lr_cls(self, testlen, ntrain, lengths, timesteps, day, attr, attry, 
        ...
        for i in range(ntrain, ntest):
            ...
            if i == ntrain:
                hsma = testdata[['code', 'date', 'ratio', 'pred', 'prob_long',
                                 'prob_short']].copy()
            else:
                hsma = pd.concat(
                    [hsma, testdata[['code', 'date', 'ratio', 'pred',
    ...
    def lr_code_cls(self, testlen, ntrain, lengths, timesteps, day, attr, attry, 
                modellabel, readfile):
        ...
        for i in range(ntrain, ntest):
            ...
            for code in traindata.code.unique():
                ...
                if (testdata_c.shape[0] == 0) | (traindata_c.shape[0] <= testdata_c.shape[0] * 3):
                    continue
                ...
                if modellabel == 'lr':
                    ...
                    testdata_c['prob_long'] = xclas.predict_proba(testdatax)[:,1]
            ...
            if i == ntrain:
                hsma = testpred[['code', 'date', 'ratio', 'pred', 'prob_long',
                                 'prob_short']].copy()
            else:
                hsma = pd.concat(
                    [hsma, testpred[['code', 'date', 'ratio', 'pred',
                                     'prob_long', 'prob_short']]],
                    ignore_index=True)
