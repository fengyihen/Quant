...
class StockShortTerm(object):
    ...
    def hsmaseq(self, timesteps):
        ...
        np.savez(
            'Test//SSTK//hsmaseq_' + str(timesteps) + '.npz',
            x=x_train,
            y=y_train)
    ...
    def hsmatrade(self, hsma, n):
        ...
        if not self.absratio:
            ...
            hsma1['closeratio'] = hsma1['closeratio'] + hsma1["ooratio" +
                                                              str(self.day)]
    ...
    def extratreestradelist(self, testlen, ntrain, ntrees, n):
        ...
        i = ntest
        traindata = hsma[(hsma['date'] >= dates[(i - ntrain) * testlen])
                         & (hsma['date'] < dates[i * testlen])].copy()
        ...
        hsmals.to_csv(
            "momentumhedge_wsd\\hedgelist\\extratreestradelist_" + self.label +
            "_" + self.enddate + ".csv",
            index=False)
