...
class FutureXgboost(FutureDay.Future):
    ...
    def xgb_reg(self, testlen, ntrain, lengths, timesteps, day, tr, attr,
                attry, feature_sel, max_depth, learning_rate, reg_alpha,
                reg_lambda, modellabel, readfile):
        ...
        for i in range(ntrain, ntest):
            ...
            ###xgboost sklearn api
            if modellabel == 'xgb':
                xclas = XGBRegressor(max_depth, learning_rate, reg_alpha,
                                     reg_lambda)  #objective='multi:softmax'
    ...
    def xgb_reg_loop(self, testlen, ntrain, lengths, timesteps, day, tr, attr,
                     attry, feature_sel, max_depths, learning_rates,
                     reg_alphas, reg_lambdas, modellabel, readfile, r, fee):
                    ...
                    for reg_lambda in reg_lambdas:
                        ...
                        if hsmaratio.shape[0] > 0:
                            temp = pd.DataFrame({
                                ...
                                reg_lambda,
                                'ratio':
                                hsmaratio.ratio.iloc[hsmaratio.shape[0] - 1]
                            },
                                                index=[0])
                            result = pd.concat([result, temp],
                                               ignore_index=True)
                            result.sort_values(
                                'ratio', axis=0, ascending=False, inplace=True)
                            print(result.head())
