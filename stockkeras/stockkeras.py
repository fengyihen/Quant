...
class StockKerasSequential(Stock):
    def hsmaseq_raw(self, timesteps):
        ...
        if not os.path.exists('Test\\stockkeras\\hsmaseq_raw_' +
                              str(timesteps) + '_' + self.label):
            os.makedirs('Test\\stockkeras\\hsmaseq_raw_' + str(timesteps) +
                        '_' + self.label)

        for code in codelist:
            traindatac = traindata[traindata['code'] == code].copy()
            traindatac = traindatac.sort_values(by='date')
            traindatac['ratio'] = traindatac['open'].shift(
                -(self.day + 1)) / traindatac['open'].shift(-1) - 1
            traindatac[
                'open'] = traindatac['open'] / traindatac['open'].shift(1) - 1
            traindatac[
                'high'] = traindatac['high'] / traindatac['high'].shift(1) - 1
            traindatac[
                'low'] = traindatac['low'] / traindatac['low'].shift(1) - 1
            traindatac['close'] = traindatac['close'] / traindatac[
                'close'].shift(1) - 1
            ...
            traindatac = traindatac[[
                'date', 'code', 'ratio', 'open', 'high', 'low', 'close',
                'free_turn'
            ]]
            print(code)
            flag = 0
            for i in range(timesteps):
                ...
                xy_temp = traindataci.values.reshape((period, timesteps,
                                                      traindataci.shape[1]))
            ...
            np.save(
                'Test\\stockkeras\\hsmaseq_raw_' + str(timesteps) + '_' +
                self.label + '\\' + code, xy_train)
    ...
    def hsmaseq(self, timesteps):
        ...
        if not os.path.exists('Test\\stockkeras\\hsmaseq_' + str(timesteps) +
                              '_' + self.label):
            os.makedirs('Test\\stockkeras\\hsmaseq_' + str(timesteps) + '_' +
                        self.label)
        ...
        for code in codelist:
            ...
            for i in range(timesteps):
                ...
                if flag == 0:
                    ...
                    xy_train = np.concatenate((xy_train, xy_temp))
            np.save(
                'Test\\stockkeras\\hsmaseq_' + str(timesteps) + '_' +
                self.label + '\\' + code, xy_train)
    ...
    def hsmacnnslice(self, traindata):
        ...
        flag = 0
        for colname in [
                'openr', 'highr', 'lowr', 'closer', 'amtr', 'freeturn'
        ]:
            cols = []
    ...
    def dnn_regression(self, testlen, ntrain, ntrees, nodes):
        ...
        #store.close()
        hsma.to_hdf(
            'Test\\stockkeras\\hsma_dnn_regression_' + self.label + '.h5',
            'hsma')
    ...
    def lstm_regression(self, testlen, ntrain, timesteps, raw):
        ...
        for i in range(ntrain, ntest):
            ...
            traindatax, traindatay = self.hsmaseqslice(timesteps, traindate0,
                                                       traindate1, raw)
            traindatay = traindatay[:, 2]
            traindatax1 = traindatax  #[:,:,[0]]
            testdatax, testdatay = self.hsmaseqslice(timesteps, testdate0,
                                                     testdate1, raw)
            ...
            model.add(
                LSTM(
                    ...
                    input_shape=(timesteps, traindatax1.shape[2]),
                    W_regularizer=l2(0.01)))  #, dropout_W=0.2, dropout_U=0.2
            model.add(Activation('relu'))
            model.add(Dropout(0.5))
            model.add(LSTM(
                32, return_sequences=True))  #, dropout_W=0.2, dropout_U=0.2
            model.add(Activation('relu'))
            model.add(Dropout(0.5))
            ...
            hsma.to_hdf(
                'Test\\stockkeras\\hsma_lstm_regression_' + self.label + '.h5',
                'hsma')
        ...
        for i in range(ntrain, ntest):
            ...
            model.add(
                LSTM(
                    ...
                    input_shape=(timesteps, traindatax1.shape[2]),
                    dropout_W=0.2,
                    dropout_U=0.2))  #, W_regularizer=l2(0.01)
