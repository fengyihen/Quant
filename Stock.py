...
class Stock():
    ...
    def __init__(self, dataset, absratio, day, length, timesteps, mtype, fee,
                 indexfee, label):
    ...
    def binandwoe_traintest(self, traindatax, traindatay, testdatax, binn, bq,
                            r0):
        #进行粗分类和woe转换
    ...
    def hsmatradedayregressor(self,
                              condition,
                              hsma,
                              n,
                              cr=None,
        ...
        if not self.absratio:
            ...
            index300 = index300[(index300.date >= hsmatradeday.date.min())
                                & (index300.date <= hsmatradeday.date.max())]
            index300.index = range(index300.shape[0])
            index300[
                '300dayratio'] = 1 - index300.close / index300.close.shift(1)
            index300.ix[
                0, '300dayratio'] = 1 - index300.close[0] / index300.open[0]
            index300['300ratio'] = index300['300dayratio'].cumsum()
            hsmatradeday = pd.merge(
                hsmatradeday, index300[['date', '300dayratio', '300ratio']])
            hsmatradeday['hedge300dayratio'] = (
                hsmatradeday.dayratio + hsmatradeday['300dayratio']) / 2
            hsmatradeday['hedge300ratio'] = (
                hsmatradeday.cumratio + hsmatradeday['300ratio']) / 2
            ...
            index500 = index500[(index500.date >= hsmatradeday.date.min())
                                & (index500.date <= hsmatradeday.date.max())]
            index500.index = range(index500.shape[0])
            index500[
                '500dayratio'] = 1 - index500.close / index500.close.shift(1)
            index500.ix[
                0, '500dayratio'] = 1 - index500.close[0] / index500.open[0]
            index500['500ratio'] = index500['500dayratio'].cumsum()
            hsmatradeday = pd.merge(
                hsmatradeday, index500[['date', '500dayratio', '500ratio']])
            hsmatradeday['hedge500dayratio'] = (
                hsmatradeday.dayratio + hsmatradeday['500dayratio']) / 2
            hsmatradeday['hedge500ratio'] = (
                hsmatradeday.cumratio + hsmatradeday['500ratio']) / 2
            ...
            if os.path.exists("Test\\strategy data\\" + cta1 + ".csv"):
                ...
                cta['cta1dayratio'] = cta.net.diff(1) / (
                    cta.open.shift(1) * 100)
                cta = cta[(cta.date >= hsmatradeday.date.min())
                          & (cta.date <= hsmatradeday.date.max())]
                cta.index = range(cta.shape[0])
                cta['cta1ratio'] = cta['cta1dayratio'].cumsum()
                hsmatradeday = pd.merge(
                    hsmatradeday, cta[['date', 'cta1dayratio', 'cta1ratio']])
                hsmatradeday['hedgecta1dayratio'] = (
                    hsmatradeday.dayratio + hsmatradeday['cta1dayratio']) / 2
                hsmatradeday['hedgecta1ratio'] = (
                    hsmatradeday.cumratio + hsmatradeday['cta1ratio']) / 2
    ...
    def tradestat(self, hsmatradeday):
        ...
        tradestat[
            'yearratio'] = tradestat['ratio'] / hsmatradeday.shape[0] * 252
        tradestat['mdd'] = mdd
        tradestat['mdddate'] = mdddate
        tradestat['RRR'] = tradestat['yearratio'] / tradestat['mdd']

        tradestat['sharpratio'] = hsmatradeday['dayratio'].mean(
        ) / hsmatradeday['dayratio'].std() * 252**0.5
