...
class Future():
    ...
    def hsmadata_raw_x(self, timestep):  #code 只能是商品
    ...
    def hsmadata_raw2_x(self, timestep):  #code 只能是商品
    ...
    def hsmadata_cr_x(self, timesteps):  #code 只能是商品
        ...
        for code in self.feelist.keys():
            ...
            temp = hsma0[['date', 'code', 'closer']].copy()
            temp.columns = ['date', 'code', 'closer_0']
    ...
    def hsmadata_y_vol(self, vn):  #code 只能是商品
        ...
        hsmadata['ROC'] = talib.ROC(hsma0.close.values, timeperiod=vn)
        hsmadata['ROC'] = hsmadata['ROC'].abs()
        hsmadata['ROC'] = hsmadata['ROC'].shift(-vn)
    ...
    def hsmadata_dayin_lsr(self, hsma, pr=0.5, lr=0.01, sr=0.01, fee=0.0004):
        ...
        hsmaratio.columns = ['dayratio']
        hsmaratio['date'] = hsmaratio.index
        temp = pd.DataFrame(hsmaall.groupby(['date'])['r'].count())
